...
/*
 * Fetch annual reports from tbl_resources.
 * catID = 'plans' based on confirmed values in the DB.
 * Ordered by infoYear DESC so newest reports appear first.
 */
$reports_qry = $dbc->prepare(
    "SELECT * FROM tbl_resources
     WHERE published = '1' AND catID = 'plans'
     ORDER BY infoYear DESC, title ASC",
);
$reports_qry->execute();
$reports = $reports_qry->fetchAll(PDO::FETCH_ASSOC);

/* Group by year for a clean chronological layout */
$reports_by_year = [];
foreach ($reports as $r) {
    $year = !empty($r["infoYear"]) ? (int) $r["infoYear"] : "Other";
    $reports_by_year[$year][] = $r;
}
    ...
    <div class="container">
        <div class="inner-page__layout">
            ...
            <main class="inner-page__content" id="main-content">
                ...
                <?php if (!empty($reports_by_year)): ?>
                    <?php foreach ($reports_by_year as $year => $items): ?>
                    <div class="downloads-section animate-on-scroll">
                        <h3 class="inner-section-title"><?php echo htmlspecialchars(
                            (string) $year,
                        ); ?></h3>
